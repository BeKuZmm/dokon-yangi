...
async def show_my_orders(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for order in orders[:10]:
        ...
        if order['debt_amount'] > 0:
            text += f" | 💰 Qarz: {order['debt_amount']:,} so'm"
    ...
    if data.startswith("add_cart_"):
        ...
        for admin_id in ADMIN_IDS:
            try:
                ...
                items_text = ""
                for item in cart_items:
                    items_text += f"  • {item['name']} x{item['quantity']} = {item['price'] * item['quantity']:,} so'm\n"
...
async def admin_reduce_debt_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if amount > target_user['total_debt']:
        await update.message.reply_text(
            "❗ Qarz miqdori {} so'm. Undan ko'p kirita olmaysiz:".format(target_user['total_debt'])
        )
        return ADMIN_REDUCE_DEBT_AMOUNT
    ...
    await update.message.reply_text(
        "✅ *{}* ning qarzi *{:,} so'm* ga kamaytirildi!\n💰 Qolgan qarz: *{:,} so'm*".format(
    ...
    try:
        await context.bot.send_message(
            target_user_id,
            "✅ Sizning qarzingiz *{:,} so'm* ga kamaytirildi!\n💰 Qolgan qarz: *{:,} so'm*".format(
...
async def admin_show_clients(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for user in users:
        ...
        debt_text = "💰 Qarz: {:,} so'm".format(debt) if debt > 0 else "Qarz yo'q"
        keyboard = InlineKeyboardMarkup([
...
async def admin_add_debt_amount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "✅ *{}* nomiga *{:,} so'm* qarz qo'shildi!\n💰 Jami qarz: *{:,} so'm*".format(
            target_user_name, amount, updated_user['total_debt']
        ),
        parse_mode='Markdown',
        reply_markup=admin_menu_keyboard()
    ...
    try:
        await context.bot.send_message(
            target_user_id,
            "📋 Sizning hisobingizga *{:,} so'm* qarz qo'shildi.\n💰 Jami qarz: *{:,} so'm*".format(
                amount, updated_user['total_debt']
            ),
            parse_mode='Markdown'
        )
